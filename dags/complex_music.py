...
default_args_dict = {
    'start_date': datetime.datetime(2022, 12, 18, 0, 0, 0),
    'concurrency': 1,
...
def _convert_to_csv(src, dest):
    data_xls = pd.read_excel(src, 'dft', dtype=str, index_col=None)
    data_xls.to_csv(dest, encoding='utf-8', index=False)


task_two = PythonOperator(
    task_id='convert_to_csv',
    dag=second_dag,
    python_callable=_convert_to_csv,
    op_kwargs={
        "src": "/opt/airflow/dags/{{ execution_date.int_timestamp }}.xlsx",
        "dest": "/opt/airflow/dags/{{ execution_date.int_timestamp }}_correct.csv"
    },
    trigger_rule='all_success',
    depends_on_past=False,
)
...
def _time_filter(previous_epoch: int, epoch: int, next_epoch: int, output_folder: str):
    ...
    df_filtered = df[(df['Modified'] >= int(previous_epoch))
                     & (df['Modified']) <= int(next_epoch)]
    df_filtered.to_csv(
        path_or_buf=f'{output_folder}/{str(previous_epoch)}_filtered.csv')
...
def _split(previous_epoch: int, output_folder: str):
    ...
    df_intavolature = df[['Piece', 'Type',
                          'Key', 'Difficulty', 'Date', 'Ensemble']]
    df_composer = df['Composer'].drop_duplicates()

    df_intavolature.to_csv(
        path_or_buf=f'{output_folder}/{str(previous_epoch)}_intavolature.csv')
    df_composer.to_csv(
        path_or_buf=f'{output_folder}/{str(previous_epoch)}_composer.csv')
